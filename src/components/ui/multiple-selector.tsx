...
export interface Option {
  ...
  fixed?: boolean;
  /** Optional tag variant for this option */
...
interface MultipleSelectorProps {
  ...
  /** Variant of the selected tag badge (default: 'default') */
  badgeVariant?: BadgeVariant;
  /**
...
const MultipleSelector = React.forwardRef<MultipleSelectorRef, MultipleSelectorProps>(
  (
    ...
    // Calculate dropdown width when open
    ...
    const CreatableItem = () => {
      ...
      if (
        isOptionsExist(options, [{ value: strippedValue, label: strippedValue }]) ||
        selected.find((s) => s.value === strippedValue)
      ) {
        return undefined;
      }
    ...
    const commandFilter = React.useCallback(() => {
      ...
      if (creatable) {
        return (value: string, search: string) => {
          return value.toLowerCase().includes(search.trim().toLowerCase()) ? 1 : -1;
        };
      }
      ...
      >
        <Popover.Root open={open} onOpenChange={setOpen}>
          <Popover.Trigger asChild>
            ...
            >
              <div className="relative flex flex-wrap gap-1 pr-7">
                {selected.map((option) => {
                  return (
                    <Badge
                      key={option.value}
                      variant={option.badgeVariant ?? badgeVariant ?? 'default'}
                      className={cn(
                    ...
                    >
                      ...
                      <button
                        ...
                        className={cn(
                          'ring-offset-background focus:ring-ring hover:bg-muted-foreground/10 hover:text-foreground ml-1 rounded-full p-1 transition-colors outline-none focus:ring-2 focus:ring-offset-2',
                ...
                <CommandPrimitive.Input
                  ...
                  className={cn(
                    'placeholder:text-muted-foreground flex-1 bg-transparent outline-none',
                    {
                      ...
                      'pr-6': selected.length !== 0, // add right padding so placeholder won't sit behind the clear button
                    },
                ...
                */}
                ...
                <button
                  ...
                  className={cn(
                    'hover:bg-muted-foreground/10 hover:text-foreground focus:ring-ring absolute right-0 flex h-6 w-6 items-center justify-center rounded-full p-0 transition-colors focus:ring-2 focus:ring-offset-2',
                    (hideClearAllButton ||
                      disabled ||
                      selected.length < 1 ||
          ...
          <Popover.Portal>
            <Popover.Content
              style={{ width: triggerWidth, zIndex: 50 }}
